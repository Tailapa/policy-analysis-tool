...
export default function Overview({
  ...
  const [selectedMiscNames, setSelectedMiscNames] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const isMisc = miscOptions.some(m => m.name === selectedMinistry);
    if (isRegBody) {
      setSelectedRegulatoryBodyNames(prev => (prev.includes(selectedMinistry) ? prev : [...prev, selectedMinistry]));
    } else if (isMisc) {
      setSelectedMiscNames(prev => (prev.includes(selectedMinistry) ? prev : [...prev, selectedMinistry]));
    } else {
  ...
  const toggleMiscName = (name: string) => {
    setSelectedMiscNames(prev => (prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]));
    setCurrentPage(1);
  };
  ...
  const filteredItems = useMemo(() => {
    ...
    return activeItems.filter(item => {
      ...
      // Miscellaneous filter (OR within selection)
      if (selectedMiscNames.length > 0 && !(item.linkedMinistries || []).some(m => selectedMiscNames.includes(m.name))) return false;
    ...
    <div className="space-y-6">
      ...
      <div className={`p-5 rounded-2xl border shadow-xl flex flex-wrap gap-3 items-center justify-between transition-all ${
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {/* Miscellaneous Filter Pill (multi-select) */}
          <div className="relative">
            <button
              onClick={() => {
                const next = !miscDropdownOpen;
                closeAllFilterDropdowns();
                setMiscDropdownOpen(next);
              }}
              className={`px-4 py-2 text-xs font-bold rounded-full flex items-center gap-1.5 border cursor-pointer transition-all shadow-sm ${
                isDark
                  ? 'bg-zinc-800 border-zinc-700 hover:bg-zinc-700 text-zinc-100'
                  : 'bg-zinc-50 border-zinc-200 hover:bg-zinc-100 text-zinc-800'
              } ${selectedMiscNames.length > 0 ? 'ring-2 ring-cyan-500' : ''}`}
            >
              <Layers size={13} className="text-cyan-500" />
              <span>{pillLabel(selectedMiscNames, 'Misc: All')}</span>
              <ChevronDown size={12} className="text-zinc-400" />
            </button>
            {miscDropdownOpen && (
              <div className={`absolute top-11 left-0 border rounded-2xl shadow-2xl w-64 max-h-60 overflow-y-auto z-30 py-1.5 text-xs transition-all ${
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 font-bold border-b text-cyan-500 ${
                    isDark ? 'hover:bg-zinc-800 border-zinc-800' : 'hover:bg-zinc-50 border-zinc-100'
                  }`}
                >
                  Clear Miscellaneous Filter
                </button>
                {miscOptions.length === 0 ? (
                  <p className="px-3 py-2 text-zinc-400">No miscellaneous entries in this issue.</p>
                ) : (
                  miscOptions.map((m) => {
                    const checked = selectedMiscNames.includes(m.name);
                    return (
                      <button
                        key={m.name}
                        onClick={() => toggleMiscName(m.name)}
                        className={`w-full text-left px-3 py-2 flex items-center gap-2 ${
                          isDark ? 'hover:bg-zinc-800' : 'hover:bg-zinc-50'
                        } ${checked ? 'bg-indigo-500/10 font-bold text-indigo-600 dark:text-indigo-400' : ''}`}
                      >
          ...
          {/* Reset Filters */}
          {(selectedMinistryNames.length > 0 || selectedRegulatoryBodyNames.length > 0 || selectedMiscNames.length > 0 || selectedThemeNames.length > 0 || selectedSubtype || selectedStatus || selectedImpact || selectedDateFilter !== 'all' || customStartDate || customEndDate || searchQuery || !isAllIssues) && (
            <button
