...
import { PolicyIntelligence } from '../../types';
import { triggerIntelligenceGeneration, fetchItemIntelligence } from '../../api';

interface PendingIntelligenceProps {
  itemId: string;
  isAdmin: boolean;
  isDark: boolean;
  onReady: (intelligence: PolicyIntelligence) => void;
}
...
export default function PendingIntelligence({ itemId, isAdmin, isDark, onReady }: PendingIntelligenceProps) {
  ...
  const poll = async (count: number) => {
    try {
      const status = await fetchItemIntelligence(itemId);
      if (status.status === 'ready' && status.intelligence) {
        setGenerating(false);
        onReady(status.intelligence);
        return;
      }
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    setPollCount(0);
    try {
      await triggerIntelligenceGeneration(itemId);
  ...
  return (
    ...
    >
      <Sparkles size={36} className={`mx-auto mb-3 ${isDark ? 'text-indigo-400' : 'text-indigo-500'}`} />
      <p className={`text-sm font-bold ${isDark ? 'text-zinc-200' : 'text-zinc-800'}`}>
        {generating ? 'Analyzing policy…' : 'Policy Intelligence not yet generated'}
      </p>
      ...
      {isAdmin && (
        ...
        >
          ...
          <span>{generating ? `Generating${'.'.repeat((pollCount % 3) + 1)}` : 'Generate Intelligence Now'}</span>
        </button>
      )}
    </div>
  );
